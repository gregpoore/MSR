...
load("../../Interim_data/raw_vsnm_cq_data_for_ml_tcga_by_seq_center_and_experimental_strategy_1Sept23.RData")
...
baseNamePerDatasetResultsFile <- "perfML_10k_tcga_WIS_raw_vsnm_cq_subsets_shuffled_1Sept23"
baseNameAllResultsFile <- "perfML_10k_tcga_WIS_raw_vsnm_cq_subsets_shuffled_ALL_1Sept23"
...
for(jj in seq_along(datasetList)){
  ...
  for(kk in seq_along(sampleTypeList)){
    ...
    for(ii in seq_along(levels(metaTmp2$disease_type))){
      ...
      scrambleFlag <- ifelse(grepl("_shuffled",metadataName), yes = TRUE, no = FALSE)
